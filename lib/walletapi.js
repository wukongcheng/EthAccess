...
walletapi.sendTransaction = (from, to, value, pwd) => {
    ...
    return web3.eth.personal.unlockAccount(from, pwd).then(() => {
        logger.debug('walletapi.sendTransaction: unlockAccount');
        ...
        logger.error('Failed to sendTransaction err=', err);
    });
};
